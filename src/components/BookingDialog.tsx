...
export function BookingProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <BookingContext.Provider value={{ showBookingDialog, setShowBookingDialog }}>
      {children}
...
function BookingDialogContent() {
  ...
  const handleSubmit = () => {
    console.log('Booking submitted:', {
      date: `${selectedDay} ${months[selectedMonth]} ${currentYear}`,
      time: selectedTime,
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={!isUpcoming}
                        className={`py-1.5 rounded-full text-sm transition-all ${
                          isSelected 
